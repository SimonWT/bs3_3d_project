...
class RotationControlNode(Node):
    def __init__(self, key_left, key_right, key_fwd, key_bwd, key_up, key_down, key_f, axis, angle=0):
        super().__init__()
        x = 0
        z = 0
        self.is_follow = False
        self.angle, self.axis, self.x, self.z = angle, axis, x, z
        self.key_left, self.key_right, self.key_fwd, self.key_bwd, self.key_up, self.key_down, self.key_f = key_left, key_right, key_fwd, key_bwd, key_up, key_down, key_f
    ...
    def draw(self, projection, view, model):
        new_view = view
        if (self.is_follow):
            new_view = lookat(vec(-4, 20, 0), vec(-4, 1, 1), vec(0, 1, 0))
        super().draw(projection, new_view, model)
...
def main():
    ...
    submarine_rot = RotationControlNode(glfw.KEY_LEFT, glfw.KEY_RIGHT, glfw.KEY_UP, glfw.KEY_DOWN, glfw.KEY_SPACE, glfw.KEY_LEFT_SHIFT, glfw.KEY_F, vec(0, 1, 0))
